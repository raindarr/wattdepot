...
public class TestVerisRowParser {
  ...
  public void testBogusColumns() {
    ...
    parsedData = parser.parseRow(row.split("\t"));
    assertNull("Row with bogus time column was successfully parsed", parsedData);
    row = "2009-05-01 00:00:03\t0\t0\t0\tEnergy Consumption (kWh)\t3.5";
    parsedData = parser.parseRow(row.split("\t"));
    assertNull("Row with bogus energyConsumed column was successfully parsed", parsedData);
    row = "2009-05-01 00:00:03\t0\t0\t0\t43770\tReal Power (kW)";
    parsedData = parser.parseRow(row.split("\t"));
    assertNull("Row with bogus powerConsumed column was successfully parsed", parsedData);
  }
