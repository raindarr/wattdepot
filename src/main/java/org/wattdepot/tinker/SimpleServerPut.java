...
public class SimpleServerPut extends Component implements Runnable {
  ...
  public void run() {
    ...
    simpleServer.getServers().add(server);
    simpleServer.getClients().add(Protocol.FILE);
    server.getContext().getParameters().add("maxThreads", "10");
  ...
  private static String callServer() throws IOException {
    ...
    Representation representation =
        new InputRepresentation(byteArrayInputStream, MediaType.APPLICATION_OCTET_STREAM);
    ...
    // Without the next line, only maxThreads connections can be made
